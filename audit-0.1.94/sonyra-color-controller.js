(function () {
	...
	function buildPatternResultCard(modal, definition) {
		...
		if (modal.mode === 'create') {
			var backButton = createButton('sonyra-manager-pages-secondary', t('manager.design.colors.pattern_back_to_choice'), 'arrow-left');
	...
	function renderModal() {
		...
		if (state.modal.kind === 'pattern-choice') {
			...
			if (state.modal.entityType === 'pattern') {
				...
				modalTitleText = t(getPatternModalTitleKey(state.modal));
				modalDescriptionText = t(getPatternModalDescriptionKey(state.modal));
			} else {
