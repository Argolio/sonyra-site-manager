...
class Sonyra_Site_Manager_Manager_Renderer {
	...
	private static function render_icon( string $name, string $class_name = 'sonyra-manager-icon' ): void {
		if ( 'site-header-footer' === $name ) {
			printf(
				'<svg viewBox="0 0 24 24" class="%1$s" aria-hidden="true" focusable="false" fill="none"><rect x="4" y="5" width="16" height="14" rx="3"></rect><path d="M7 8h10"></path><path d="M7 16h10"></path><path d="M7 11h3"></path><path d="M12 11h2"></path><path d="M16 11h1"></path></svg>',
				esc_attr( 'sonyra-icon ' . $class_name )
			);
			return;
		}
	...
	private static function render_page_header( array $section ): void {
		$route                = isset( $section['route'] ) ? (string) $section['route'] : '';
		$pages_actions_hidden = 'pages' === $route ? '' : ' hidden';
		$setup_actions_hidden = 'setup' === $route ? '' : ' hidden';
		...
		<header class="sonyra-manager-topbar sonyra-manager-page-header">
			...
			<div>
				<div class="sonyra-manager-page-header-actions" data-manager-header-actions="setup"<?php echo $setup_actions_hidden; ?>>
					<a class="sonyra-manager-pages-secondary" href="<?php echo esc_url( $site_url ); ?>" target="_blank" rel="noopener noreferrer">
						...
						<span><?php echo esc_html( self::text( 'manager.site.actions.open_site' ) ); ?></span>
					</a>
				</div>
				<div class="sonyra-manager-page-header-actions" data-manager-header-actions="pages"<?php echo $pages_actions_hidden; ?>>
	...
	private static function render_site_view(): void {
		...
		<section class="sonyra-manager-view sonyra-manager-view-site" data-manager-view="setup" hidden>
			<div class="sonyra-manager-site-foundation">
				<?php self::render_site_primary_tool_card(); ?>
			</div>
		</section>
		<?php
	}

	private static function render_site_primary_tool_card(): void {
		$chips = array(
			'manager.site.header_footer_chip_header',
			'manager.site.header_footer_chip_menu',
			'manager.site.header_footer_chip_footer',
			'manager.site.header_footer_chip_contacts',
		);
		?>
		<article class="sonyra-manager-site-tool-card sonyra-manager-site-tool-card--primary">
			<div class="sonyra-manager-site-tool-card-head">
				<div class="sonyra-manager-site-tool-card-icon">
					<?php self::render_icon( 'site-header-footer', 'sonyra-manager-icon sonyra-manager-card-icon' ); ?>
				</div>
				<div class="sonyra-manager-site-tool-card-copy">
					<h2 class="sonyra-manager-site-tool-card-title"><?php echo esc_html( self::text( 'manager.site.header_footer_site_title' ) ); ?></h2>
					<p class="sonyra-manager-site-tool-card-description"><?php echo esc_html( self::text( 'manager.site.header_footer_site_description' ) ); ?></p>
					<div class="sonyra-manager-site-tool-card-chips">
					<?php foreach ( $chips as $chip_key ) : ?>
						<span class="sonyra-manager-page-badge sonyra-manager-page-badge-neutral">
							<span><?php echo esc_html( self::text( $chip_key ) ); ?></span>
						</span>
					<?php endforeach; ?>
					</div>
				</div>
			</div>
			<div class="sonyra-manager-site-tool-card-footer">
				<button type="button" class="sonyra-manager-pages-primary sonyra-manager-site-tool-card-action" data-site-tool="header-footer">
					<span data-pages-button-icon="settings"><?php self::render_icon( 'settings', 'sonyra-manager-icon sonyra-manager-pages-button-icon' ); ?></span>
					<span><?php echo esc_html( self::text( 'manager.site.actions.configure' ) ); ?></span>
				</button>
			</div>
		</article>
		<?php
